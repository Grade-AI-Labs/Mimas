...
Create a product requirements document through structured interview, then submit it as a GitHub Issue.
...
After writing the PRD, submit it as a GitHub Issue:

- Use `gh issue create` with the PRD content as the body
- **Title**: A concise imperative description of the feature (e.g. "Add notification preferences for users")
- **Body**: The full PRD content in markdown
- **Labels**: Add relevant labels if they exist (e.g. `enhancement`, `feature`). Check available labels with `gh label list` first — do not create labels that don't exist.
- Ask the user if they want to assign it to anyone or add it to a project/milestone
