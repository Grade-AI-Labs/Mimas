...
description: Populate `docs/features/<slug>.md` for one, several, or every undocumented feature area by dispatching up to 10 parallel subagents — one per feature. Use whenever the user wants to document features, fill out feature docs, write up specific features (e.g. "document auth and billing"), document all undocumented features, or follow up on `find-features` discovery. This is the natural sequel to `find-features`: that skill identifies what is missing, this skill writes the docs in parallel.
metadata:
  author: Olof Brogeby
...
Write feature documentation for the repository — one populated `docs/features/<slug>.md` per requested feature — by dispatching subagents in parallel so an "all" run does not serialize.
...
- `docs/features/feature-template.md` — canonical template
...
Read `docs/AGENTS_FEATURES.md` and the root `AGENTS.md` (plus any subdomain `CONTEXT.md` files) so you know the contract — what counts as an area-level doc vs. a per-service doc, and which subdomains exist. These files are authoritative; if anything in this skill drifts from them, the files win.
...
Each subagent gets a self-contained prompt — it has not seen this conversation, so include everything it needs:

```
You are documenting a single feature area for this repository. The output is one populated markdown file at `docs/features/<slug>.md` that follows the project's feature documentation contract.

Feature slug: <slug>
One-line concept (from discovery, may be rough): <concept>

Authoritative reading order (read these first, in order):
1. `docs/AGENTS_FEATURES.md` — the contract that governs how feature docs are written
2. `docs/features/feature-template.md` — the canonical template. Mirror its section order and headings; do not invent your own structure
...
- Create `docs/features/<slug>.md` from the template
...
- If the feature has multiple distinct services or endpoints worth separating, also create `docs/features/<slug>/<service>.md` files from the same template and link them from the area doc — but only when complexity actually warrants it. Do not duplicate large sections between area and per-service docs
- Do not edit `docs/FEATURES.md` — the dispatcher will update the index once all features are written, to avoid concurrent edits to the same file
...
Substitute `<slug>` and `<concept>` per feature. Keep the rest verbatim — the subagent depends on having the contract in its own context.

If features turn out to be unrelated to one another (different subdomains, different layers), there's no need to coordinate beyond avoiding the shared `docs/FEATURES.md` file. The dispatcher updates the index after the batch returns.

---

## Phase 4 — Update the index and report back

Once a batch completes, collect each subagent's reported one-liner and add entries to `docs/FEATURES.md` in alphabetical order, in the form:
...
- Which requested features were skipped, and why ("already documented at `docs/features/<slug>.md`", "could not locate in codebase", "implementation too thin to earn an entry yet")
- Which sections in the new docs are TODOs that still need human judgment
- That `docs/FEATURES.md` was updated and how many entries were added
- Any discrepancies you noticed between `docs/FEATURES.md` and the filesystem
